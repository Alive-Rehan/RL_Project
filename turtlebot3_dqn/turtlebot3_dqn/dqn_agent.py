...
class TurtlebotGymEnv(gym.Env):
    def __init__(self, dqnAgent: DQNAgent):
        ...
        # Define observation space (e.g., agent's position as a box)
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(1,dqnAgent.state_size), dtype=float)
    ...
    def step(self, action):
        ...
        terminated = done # Episode ends when goal reached (add goal logic)
        truncated = done # For time limits (can use gym.wrappers.TimeLimit)
